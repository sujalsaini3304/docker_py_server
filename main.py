...
#upload from youtube link using tempfile
@app.post("/api/upload/youtube/url/tempfile")
async def upload_from_youtube_link__via_tempfile(data: YouTubeURL):
    client = None
    try:
        ...
        with yt_dlp.YoutubeDL({'format': '140'}) as ydl:
            ...
            year = info.get('upload_date', None)
            if year:
                year = int(year[:4])
        ...
        with tempfile.NamedTemporaryFile(suffix=".mp3", delete=True) as tmp_file:
            ffmpeg_cmd = [
                'ffmpeg', '-i', audio_url,
                '-f', 'mp3', '-vn', '-acodec', 'libmp3lame', '-ab', '128k',
                '-y', tmp_file.name
            ]
...
#upload from youtube link using ByteIO
@app.post("/api/upload/youtube/url/byteio")
async def upload_from_youtube_link_via_byteio(data : YouTubeURL):
     client = None
     try:
        ...
        upload_result = cloudinary.uploader.upload(
            mp3_data, resource_type="auto", folder = "my-music-web-app/data/assets/"
        )
